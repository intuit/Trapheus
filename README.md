...
2. AWS Credentials configuration. Trapheus uses boto3 as client library to talk with Amazon Web Services. Feel free to [use any environment variables](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/credentials.html#environment-variables) that boto3 supports to configure credentials can be used to supply the Auth. Inside the lambda, Trapheus will use the IAM credentials from IAM role that Lambda is running under.
